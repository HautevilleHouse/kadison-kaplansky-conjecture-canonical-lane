...
In-paper anchor: `paper/KADISON_KAPLANSKY_CONJECTURE_PREPRINT.md` (`KK_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving absence of nontrivial projections in reduced group `C^*`-algebras through an admissible spectral-idempotent closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`KK_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and projection windows do not collapse.
This is the first-failure capture contribution to `M_KK`.
...
If near-failure extraction and no-Zeno spacing hold, then `KK_G3` prevents hidden drift before rigidity is applied.
