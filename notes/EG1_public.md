...
In-paper anchor: `paper/KADISON_KAPLANSKY_CONJECTURE_PREPRINT.md` (`KK_G1`).

## Goal
Make the projected response floor explicit as the protected-core gate for `proving absence of nontrivial projections in reduced group `C^*`-algebras through an admissible spectral-idempotent closure architecture`.
...
- projected core: the response sector controlled by `kappa_projection`.
...
`KK_G1` closes when `kappa_projection` satisfies the response-floor requirement: projected operator response has a strict positive floor.
This is the protected-core contribution to the strict margin `M_KK`.
...
Payload: verify that all quantities used by `kappa_projection` are defined on the projected sector named in the main preprint.
...
Payload: check the artifact key `kappa_projection` and the corresponding extraction input/provenance record.

### Theorem EG1.3 (core gate closure)
If Lemmas EG1.1-EG1.2 hold and the runtime artifact accepts `kappa_projection`, then `KK_G1` supplies the projected/protected-core input to `M_KK`.

## Current Instantiation

- gate: `KK_G1`
- artifact key: `kappa_projection`
