...
In-paper anchor: `paper/KADISON_KAPLANSKY_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving absence of nontrivial projections in reduced group `C^*`-algebras through an admissible spectral-idempotent closure architecture`.
...
4. the strict closure margin `M_KK` remains positive.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_KK` is meaningful.
